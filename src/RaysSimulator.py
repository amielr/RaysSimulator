...
def get_fourier_frequencies(function):
...
def wigner_function(function, space, frequencies):
    # print("iteration")
    # print(function)
    corrmatrix = prep_corr_matrix_from_vector(function)
...
def wignerize_each_function(functionobject):
    ...
    rowspace, rowfrequencies = get_meshgrid(xinterpaxis,
                                           rowfreqs)  # build axis dimensions correlating space and phase for rows of function
    colspace, colfrequencies = get_meshgrid(yinterpaxis,
                                           colfreqs)  # build axis dimensions correlating space and phase for columns of function
...
def build_ray_matrices(wignerobject, transformedwignerobject, functionobject):
...
mirrorobject, mirrorinterpolator = create_interpolated_mirror()
# print("the mirror distance function is")
# print(mirrorobject[2])

# plot3dto2d(mirrorobject[0],mirrorobject[1],mirrorobject[2])

functionobject = input_function()  # returns x , y and the Function value/amplitude
...
zwignerobject = wignerize_each_function(functionobject)  # wigner amp, space, frequency

# wignerobject = abs(wignerobject)

plot_3d_to_2d(zwignerobject[0][5][1], zwignerobject[0][5][2],
           zwignerobject[0][5][0])  # pass in space, phase and wigner amplitude

zwignerobjecttrans = ray_transforms(zwignerobject, 50)

# plot3dto2d(wignerobjecttrans[0][5][1],wignerobjecttrans[0][5][2],wignerobjecttrans[0][5][0])   #pass in space, phase and wigner
reversefunction = integrate_intensity_wig(zwignerobject, functionobject)
...
rayobject = build_ray_matrices(zwignerobject, zwignerobjecttrans, functionobject)
